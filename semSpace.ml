...
let fR2 = [("person",["1";"2";"3"]);
	      ("number",["sg";"pl"]);
	      ("class",["human";"nonhuman"])
	     ];;

(*specify pairs of feature values that cannot co-occur*)
let ruledOut = [(("person","1"),("class","nonhuman"))];;

(*run this function to produce a text file called "text.txt" which contains the list of all possible environments*)
let space = getSemanticSpace fR2 ruledOut in
produce_txtfile space;;

(*Open the file "text.txt" and insert affixes associated with each environment *immediately to the left of the environment as one string followed by a space. Do not change the order of the rows! (this is crucial for generating the right text).* Save the resulting file under some name, and use that name in the function below. *)

let text = make_text "filename.txt" (getSemanticSpace fR2 ruledOut);;
 
(*    
test:
let text = make_text "text2.txt" space;;
